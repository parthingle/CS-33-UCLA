...
//SHOULD BE COMPLETELY OPTIMIZED
void func0(double *weights, double *arrayX, double *arrayY, int xr, int yr, int n)
{
	int i;
	double x = 1/((double)(n));//hoisted
	double dx = xr;//only have the ints casted once
	double dy = yr;
	//typical multithreaded for loop
	
	//split the loops to make use of locality
			...
			int Ones, int iter, int X, int Y, int Z, int n)
{
	...
	//need private keyword because declaration is outside loop
...
//left unparallelized, it seems I cannot parallelize this function
